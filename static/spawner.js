...
const populateList = async (dom) => {
  ...
  if (dom) {
    ...
    list.append(
      ...entities.map((entity) => {
        const texture = new TextureSet(entity);
        if (texture && texture.id >= 0 && Object.keys(entity['Additional']).length === 0) {
          texture.getDOM().then((tex) => {
            document.querySelector("a[href=\"#"+entity['Name']+"\"]").prepend(tex())
          });
        }
        return elem("a", {
          className: "item list-group-item list-group-item-action",
          href: "#" + entity['Name'],
          innerHTML: "<b>"+entity['ID']+"</b>: "+entity['Name']
        });
      })
    );
  }
